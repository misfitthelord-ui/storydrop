...
export default function Play() {
  ...
  async function loadChapter(ch) {
    setLoading(true)
    setLoadingMsg(`Claude is writing chapter ${ch}...`)
    setStoryText('')
    setChoices(null)
    setSelected(null)
    setConsequence('')
  ...
  async function choose(key, text) {
    setSelected(key)
    choiceMade.current = text
    setScore(s => s + chapter * 15)
    setLoading(true)
    setLoadingMsg('Writing the consequence...')

    try {
      const result = await callAPI({ type: 'consequence', genre, storyContext: storyContext.current, choiceMade: text, chapter })
      if (!result) return
      storyContext.current += ' ' + result
      setConsequence(result)
    } catch (e) {
      setConsequence('Your choice echoes through the story...')
    }
    setLoading(false)
  }
  ...
  async function finishStory() {
    ...
    try {
      ...
      setEnding({
        title: titleMatch ? titleMatch[1].trim() : 'Your story ends',
        text: endingMatch ? endingMatch[1].trim() : raw.trim()
      })
      setPhase('end')
    } catch (e) {
      setError('Could not generate ending. Try again.')
    }
    setLoading(false)
  }
  ...
  const S = {
    ...
    logoName: { fontSize: 16, fontWeight: 500, color: '#fff' },
    ...
    choiceBtn: (picked) => ({
      width: '100%', background: picked ? 'rgba(83,74,183,0.25)' : 'rgba(255,255,255,0.04)',
      border: picked ? '1px solid #534AB7' : '0.5px solid rgba(255,255,255,0.12)',
      borderRadius: 12, padding: '14px 16px', cursor: picked ? 'default' : 'pointer',
      display: 'flex', alignItems: 'center', gap: 12, textAlign: 'left',
      fontSize: 14, color: picked ? '#AFA9EC' : 'rgba(255,255,255,0.7)',
      marginBottom: 10, transition: 'all .15s'
    }),
    letter: (picked) => ({
      width: 26, height: 26, borderRadius: '50%',
      background: picked ? '#534AB7' : 'rgba(255,255,255,0.08)',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      fontSize: 12, fontWeight: 500, color: picked ? '#fff' : 'rgba(255,255,255,0.5)', flexShrink: 0
    }),
    ...
    prog: { height: 3, background: 'rgba(255,255,255,0.08)', borderRadius: 2, marginBottom: 24, maxWidth: 600, margin: '0 auto 24px' },
  }
  ...
  return (
    <div style={S.page}>
      <div style={S.topbar}>
        ...
        <span style={S.logoName}>StoryDrop</span>
      ...
      {upgradeFlash && (
        <div style={{ maxWidth: 600, margin: '0 auto 20px', background: 'rgba(29,158,117,0.15)', border: '0.5px solid #1D9E75', borderRadius: 10, padding: '12px 16px', fontSize: 14, color: '#5DCAA5' }}>
          Welcome to Pro! Unlimited stories unlocked.
        </div>
      )}
      ...
      {phase === 'setup' && (
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
            {GENRES.map(g => (
              <div key={g.id} onClick={() => setGenre(g.id)} style={{
                background: genre === g.id ? 'rgba(83,74,183,0.2)' : 'rgba(255,255,255,0.04)',
                border: genre === g.id ? '1.5px solid #534AB7' : '0.5px solid rgba(255,255,255,0.1)',
          ...
          <button onClick={startGame} style={{ width: '100%', background: '#534AB7', color: '#fff', border: 'none', borderRadius: 12, padding: 16, fontSize: 16, fontWeight: 500, cursor: 'pointer' }}>
            Generate my story →
          </button>
        </div>
      )}
      ...
      {phase === 'playing' && (
        <>
          <div style={{ ...S.prog, maxWidth: 600 }}><div style={{ height: '100%', background: '#534AB7', borderRadius: 2, width: `${Math.round((chapter / MAX_CHAPTERS) * 100)}%`, transition: 'width .5s' }} /></div>
          <div style={S.card}>
                ...
                <>
                  ...
                  {consequence && (
                    <div style={S.resultBox}>
                      <div style={{ fontSize: 11, color: '#7F77DD', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.08em' }}>Consequence</div>
                      <div style={{ fontSize: 14, color: '#AFA9EC', lineHeight: 1.7 }}>{consequence}</div>
                    </div>
                  )}

                  {consequence && !loading && (
                    <button style={S.nextBtn} onClick={nextChapter}>
                      {chapter >= MAX_CHAPTERS ? 'See my ending →' : 'Continue story →'}
                    </button>
                  )}
      ...
      {phase === 'end' && ending && (
        <div style={{ maxWidth: 600, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'inline-flex', background: 'rgba(83,74,183,0.2)', border: '0.5px solid #534AB7', borderRadius: 12, padding: '12px 28px', marginBottom: 32 }}>
            <div>
              <div style={{ fontSize: 32, fontWeight: 500, color: '#AFA9EC' }}>{score}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)' }}>story points</div>
            </div>
          </div>
